...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
	...
	protected internal string mAppVersionPun
	{
		get
		{
			return string.Format("{0}_{1}", PhotonNetwork.gameVersion, "1.70");
		}
	}
	...
	public void OnEvent(EventData photonEvent)
	{
		...
		case 206:
		{
			...
			short num8 = num6;
			while (num8 - num6 < hashtable3.Count - num7)
			{
				OnSerializeRead(hashtable3[num8] as object[], photonPlayer, networkTime, correctPrefix);
				num8++;
			}
			return;
		}
		...
		case 207:
		{
			...
			int num9 = (int)hashtable2[(byte)0];
			if (num9 >= 0)
			{
				DestroyPlayerObjects(num9, true);
				return;
			}
	...
	public void RunViewUpdate()
	{
		...
		{
			if (value3.synchronization == ViewSynchronization.Off || !value3.isMine || !value3.gameObject.activeInHierarchy || blockSendingGroups.Contains(value3.group))
			{
				continue;
			}
			...
			if (value3.synchronization == ViewSynchronization.ReliableDeltaCompressed || value3.mixedModeIsReliable)
			{
				...
				{
					value = new ExitGames.Client.Photon.Hashtable(10);
					dataPerGroupReliable[value3.group] = value;
				}
				value.Add((short)(value.Count + 10), array);
			}
			else
			{
				...
				{
					value2 = new ExitGames.Client.Photon.Hashtable(10);
					dataPerGroupUnreliable[value3.group] = value2;
				}
				value2.Add((short)(value2.Count + 10), array);
			}
		}
		RaiseEventOptions raiseEventOptions = new RaiseEventOptions();
		foreach (int key in dataPerGroupReliable.Keys)
		{
			...
			OpRaiseEvent(206, hashtable, true, raiseEventOptions);
			hashtable.Clear();
		}
		foreach (int key2 in dataPerGroupUnreliable.Keys)
		{
			...
			OpRaiseEvent(201, hashtable2, false, raiseEventOptions);
			hashtable2.Clear();
		}
	}
