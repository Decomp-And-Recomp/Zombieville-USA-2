...
namespace ExitGames.Client.Photon
{
	internal class NCommand : IComparable<NCommand>
	{
		...
		internal byte[] SerializeHeader()
		{
			...
			commandHeader = new byte[SizeOfHeader];
			commandHeader[0] = commandType;
			commandHeader[1] = commandChannelID;
			commandHeader[2] = commandFlags;
			commandHeader[3] = reservedByte;
			int targetOffset = 4;
			Protocol.Serialize(Size, commandHeader, ref targetOffset);
			Protocol.Serialize(reliableSequenceNumber, commandHeader, ref targetOffset);
			if (commandType == 7)
			{
				targetOffset = 12;
				Protocol.Serialize(unreliableSequenceNumber, commandHeader, ref targetOffset);
			}
			else if (commandType == 8)
			{
				targetOffset = 12;
				Protocol.Serialize(startSequenceNumber, commandHeader, ref targetOffset);
				Protocol.Serialize(fragmentCount, commandHeader, ref targetOffset);
				Protocol.Serialize(fragmentNumber, commandHeader, ref targetOffset);
				Protocol.Serialize(totalLength, commandHeader, ref targetOffset);
				Protocol.Serialize(fragmentOffset, commandHeader, ref targetOffset);
			}
			return commandHeader;
		}
